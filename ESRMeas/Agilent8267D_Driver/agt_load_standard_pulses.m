function agt_load_standard_pulses(sequence)
...
for ip = 1:length(defined_pulses)
  if any(defined_pulses(ip) == sequence) > 0 % check if given pulse (ip) is used in the "sequence"
     create_load_pulse(p(ip),amplitude); % creat and load the pulse in case it is used
  end;
end;
...
%   agt_load_pulse(p0x_IQ_data, p0x_name, p0x_markers);
  ...
  agt_load_pulse(p1w_IQ_data, p1w_name, p1w_markers);
  agt_load_pulse(p0u_IQ_data, p0u_name, p0u_markers);
