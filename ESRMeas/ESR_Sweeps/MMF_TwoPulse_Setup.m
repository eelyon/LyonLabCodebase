...
srt = parameters.srt;%2*ms; % shot repetition time
...
% one can use nrepeat for shot repetition or averages (limit of 100 for
% averages). Averages stores data on the buffer so it's faster but limited
% in shot number.
% TODO: test to make sure averages actually scales data properly. nrepeat
% clearly does not.
...
% disp([int2str(shotNum) ' of ' int2str(numShots)]);
...
%saveData(IQPlot,'MMF_He_3_Cryo_IQ');
disp('Done!');
