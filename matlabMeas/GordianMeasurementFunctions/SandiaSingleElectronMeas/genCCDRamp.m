function genCCDRamp(awg1, awg2, awg3, varargin)
    ...
    p.addParameter('ramp_frac', 1/3, @(x) isnumeric(x) && x>=0 && x<=0.5);
    p.parse(varargin{:});
    ...
    function r = ramp_up(N)
        if N==0, r=[]; return; end
        v = linspace(0,1,N+2); r = v(2:end-1);
    end
    function r = ramp_down(N)
        if N==0, r=[]; return; end
        v = linspace(1,0,N+2); r = v(2:end-1);
    end

    function pulse = make_pulse(HALF, RAMP)
        % Standard: ramp_up | flat_HI | ramp_down | flat_LO
        pulse = [ramp_up(RAMP), ones(1,HALF-RAMP), ramp_down(RAMP), zeros(1,HALF-RAMP)];
    end
    ...
    pulse_s_starthi = make_pulse_start_hi(HALF_s, RAMP_s);
    ...
    function pulse = get_pulse(g)
        switch gate_type{g}
            ...
            case 's_shi',   pulse = pulse_s_starthi;
