...
function updatePlots(plotHandles,xvals,mag,phase,x,y,doBackAndForth,index,halfway)
    if doBackAndForth && index <= halfway
        setXYData(plotHandles{1},xvals,x);
        setXYData(plotHandles{3},xvals,y);
        setXYData(plotHandles{5},xvals,mag);
        setXYData(plotHandles{7},xvals,phase);
    elseif doBackAndForth && index > halfway
        setXYData(plotHandles{1},xvals(1:halfway),x(1:halfway));
        setXYData(plotHandles{3},xvals(1:halfway),y(1:halfway));
        setXYData(plotHandles{5},xvals(1:halfway),mag(1:halfway));
        setXYData(plotHandles{7},xvals(1:halfway),phase(1:halfway));
    
        setXYData(plotHandles{2},xvals(halfway+1:end),x(halfway+1:end));
        setXYData(plotHandles{4},xvals(halfway+1:end),y(halfway+1:end));
        setXYData(plotHandles{6},xvals(halfway+1:end),mag(halfway+1:end));
        setXYData(plotHandles{8},xvals(halfway+1:end),phase(halfway+1:end));
    else
        setXYData(plotHandles{1},xvals,x);
        setXYData(plotHandles{2},xvals,y);
        setXYData(plotHandles{3},xvals,mag);
        setXYData(plotHandles{4},xvals,phase);
    end
end

function setXYData(plotHandle,xDat,yDat)
