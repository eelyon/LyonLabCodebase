function [optimalValue] = sweepOptimize(device, doorDevice, stepSize, start, stop, type)
N = 1:length(stepSize);
for i = N
    delta = stepSize(i);
...
if minimumValue == stop && strcmp(type,'Amp')
    ...
    newStart = stop;
    newStop = stop + delta;
    optimalValue = sweepOptimize(device, doorDevice, stepSize, newStart,newStop, type);
else
