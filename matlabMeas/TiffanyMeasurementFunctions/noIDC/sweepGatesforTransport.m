...
if strcmp(sign,'Pos')
    ...
    for i = 1:length(step)
        ...
        sigDACRampVoltage(device,[DoorInPort,TwidPort,SensPort,DoorOutPort],[volt-0.5,volt+0.5,volt+0.5,doorOutVolt],numSteps*5);
        sigDACRampVoltage(device,TopPort,-0.7+(0.5*i),numSteps*5);
    end
else
    step = 0:-0.5:finalVoltage+0.5;
    thinFilmVolt= -1; % getVal(controlDAC,TfVolt);
    for i = 1:length(step)
        volt = step(i);
        ...
        sigDACRampVoltage(device,[STOuterPort,STMidPort,STInnerPort],[volt-0.5,0,volt-0.5],numSteps*5);
    end
end
end