...
% let in electrons
sigDACRampVoltage(controlDAC,[STOBiasEPort,StmEPort,STIBiasEPort],[STVoltage-0.5,STVoltage-0.3,STVoltage-0.2],numSteps);
...
% let in electrons
sigDACRampVoltage(controlDAC,[STOBiasCPort,StmCPort,STIBiasCPort],[STCVoltage-0.4,STCVoltage-0.3,STCVoltage-0.2],numSteps);
...
sweep1DMeasSR830({'TWW'},STCVoltage,STCVoltage-0.5,0.05,0.1,10,{SR830Twiddle},controlDAC,{TwiddleCPort},1,1);
sweep1DMeasSR830({'SEN'},STCVoltage,STCVoltage-0.3,0.05,0.1,10,{SR830TwiddleC},controlDAC,{SenseCPort},1,1);

disp(i)
end

%% other side (HEMT2) with MFLI lock-in
repeat = 3;
for i = 1:repeat
% get rid of electrons    
STCVoltage = -6;
...
sigDACRampVoltage(controlDAC,[STOBiasCPort,StmCPort,STIBiasCPort],[STCVoltage+0.5,STCVoltage+0.5,STCVoltage+0.5],numSteps);
MFLISweep1D_getSample('Door',STCVoltage-1,STCVoltage,-0.05,'dev32021',controlDAC,DoorCInPort,1,'time_constant',0.3)
sigDACRampVoltage(controlDAC,[STOBiasCPort,StmCPort,STIBiasCPort],[STCVoltage,STCVoltage,STCVoltage],numSteps);
...
sweep1DMeasSR830({'Door'},-0.3,-1,0.1,0.1,10,{SR830TwiddleC},controlDAC,{DoorCInPort},0,1);
sigDACRampVoltage(controlDAC,[STOBiasCPort,StmCPort,STIBiasCPort],[0,0,0],numSteps);
MFLISweep1D_getSample('TWW',STCVoltage,STCVoltage-0.5,-0.05,'dev32021',controlDAC,TwiddleCPort,1,'time_constant',0.3)
sweep1DMeasSR830({'SEN'},STCVoltage,STCVoltage-0.3,0.05,0.1,10,{SR830TwiddleC},controlDAC,{SenseCPort},1,1);
