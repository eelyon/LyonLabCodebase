...
h = figure;
    VIDCNG     = getVal(IdcNGDevice,IdcNGPort);
    VIDCNF     = getVal(IdcNFDevice,IdcNFPort);
    VIDCN      = VIDCNG+VIDCNF;
    VIDCPG     = getVal(IdcPGDevice,IdcPGPort);
    VIDCPF     = getVal(IdcPFDevice,IdcPFPort);
    VIDCP      = VIDCPG+VIDCPF;
    VIDCAvg    = (VIDCN+VIDCP)/2;
    VIDCDel    = num2str(VIDCP-VIDCN);
suptitle(['IDC Delta =' ' ' VIDCDel])
...
for Vout = V
    ...
    Vout3 = VIDCNG+Vout/2;
    Vout4 = VIDCNF+Vout/2;
    VIDCAvg = VIDCAvg+Vout;
    
    err1 = setVal(IdcPGDevice,IdcPGPort,Vout1);
    err2 = setVal(IdcPFDevice,IdcPFPort,Vout2);
    err3 = setVal(IdcNGDevice,IdcNGPort,Vout3);
    err4 = setVal(IdcNFDevice,IdcNFPort,Vout4);
