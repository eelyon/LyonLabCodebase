...
#define print2(x,y) (SerialUSB.print(x), SerialUSB.println(y))
#define print2d(p1,x,p2,y) (SerialUSB. print(p1), SerialUSB.print(x),SerialUSB.print(p2), SerialUSB.println(y))
#define MAX_RESOLUTION 18
const int numberOfDACChans = 24;
#define CR '\r'
#define LF '\n'
#define BS '\b'
#define NULLCHAR '\0'
...
int setDACvoltage(int channel, long int);  // Function prototype, its declaration
...
const char *vgndCommandToken     = "VGND";
const char *idCommandToken       = "*IDN?";                          //Used to identify device
const char *helpCommandToken     = "HELP";
const char *verboseCommandToken  = "VERBOSE";                   //turn on or off verbose mode
const char *initCommandToken     = "INIT";                         //Initialize the dac registers and set voltage output to zero volts
...
int verboseset() {
  verbosestatus = readNumber(); // sets the current channel
...
DoMyCommand(char * commandLine) {
  ...
  { //Modify here
    ...
    result = testcmd();
    SerialUSB.println("Identify voltages set to channel * 0.1V (neglecting voltage dividers)");
  }
  else if (strcmp(ptrToCommandName, vgndCommandToken) == 0) {           //Modify here
    result = vgndcmd();
    if (verbosestatus == 1) {
      print2(">   All voltages set to zero", result);
    }
  }
  else if (strcmp(ptrToCommandName, voltreadCommandToken) == 0) {           //Modify here
    resultd = getvoltagecmd();
    SerialUSB.println(resultd);
    ...
    SerialUSB.println("SigillitoDAC_V2_SN001 built May 2022");
  }
  else if (strcmp(ptrToCommandName, helpCommandToken) == 0) {           //Modify here
    //result = setvoltagecmd();                                 //K&R string.h  pg. 251
    SerialUSB.println("Commands:  CH XX - sets channel -- CH? - query channel -- VOLT X.XXXXXXX -- sets voltage for selected channel -- VOLT? - query voltage -- TEST - sets channel to channel number divided by 10 volts -- VGND - set all channels to virtual ground of zero -- *IDN? - query device name");
  }
  else if (strcmp(ptrToCommandName, initCommandToken) == 0) {           //Modify here
    initDACS();
    if (verbosestatus == 1) {
      SerialUSB.println("All DACs initialized and ungrounded with output of 0V");
    }
  }
  else if (strcmp(ptrToCommandName, verboseCommandToken) == 0) {           //Modify here
    result = verboseset();
    if (verbosestatus == 1) {
      SerialUSB.println("Verbose mode turned on - this is good for debugging but will slow things down");
    }
    else {
      SerialUSB.println("Verbose mode turned off - this is good for running fast");
    }
    ...
    if (numChans == 0){                           //reads voltages of all channels
      int totalChan = numberOfDACChans; 
