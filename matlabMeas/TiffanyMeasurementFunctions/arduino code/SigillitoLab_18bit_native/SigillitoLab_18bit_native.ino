...
const int clockdivider = 25; // due has 84MHz clock, div by 21 gives 4MHz clock rate which is reasonable
...
void setup(){
  ...
  SerialUSB.begin(250000); // in the end we won't need serial but it is useful for debugging.
...
long AD5791_SetRegisterValueDoor13(int channel, unsigned char registerAddress, unsigned long registerValue) {
  ...
  REG_PIOC_CODR |= (0x01 << 1);  // set LOW, clear pin
  status = SPI.transfer(writeCommand[0]);
  status = SPI.transfer(writeCommand[1]);
  status = SPI.transfer(writeCommand[2]);
  REG_PIOC_SODR |= (0x01 << 1);  //set HIGH, set pin
...
long AD5791_SetRegisterValueDoor15(int channel, unsigned char registerAddress, unsigned long registerValue) {
  ...
  REG_PIOD_CODR |= (0x01 << 10);
  status = SPI.transfer(writeCommand[0]);
  status = SPI.transfer(writeCommand[1]);
  status = SPI.transfer(writeCommand[2]);
  REG_PIOD_SODR |= (0x01 << 10);
...
int initDACS() // initializes dac by setting control register and defining a voltage (currently 0 V, may eventually want to reverse command to set voltage first or init to ground)
{
   ...
   for(int channel=1;channel<=24;channel++)
      ...
      long int value = maxBit*(voltage+10)/20; // convert voltage to "DAC code" (see page 21 of datasheet)
...
int setDACvoltage(int channel, long int value)
{
  long status = AD5791_SetRegisterValue(channel, AD5791_REG_DAC, value); // here is where we set and update value of DAC.
  return 0;
}

int setDACvoltageDoor13(int channel, long int value)
{
  long status = AD5791_SetRegisterValueDoor13(channel, AD5791_REG_DAC, value); // here is where we set and update value of DAC.
  return 0;
}

int setDACvoltageDoor15(int channel, long int value)
{
  long status = AD5791_SetRegisterValueDoor15(channel, AD5791_REG_DAC, value); // here is where we set and update value of DAC.
  return 0;
}

int setDACcode(int channel, long int code) // initializes dac by setting control register and defining a voltage (currently 0 V, may eventually want to reverse command to set voltage first or init to ground)
{
  // add conditional here to check voltage value is in range.
  if(code > maxBit)
  {
   code = maxBit;
   //Serial.println("Holy shit that was close... you just tried to make the DAC adjust it's output by 20V... that wasn't very smart");
  }

  if(code < 0)
  {
   code = 0;
   SerialUSB.println("what are you doing? don't set negative dac codes.");
  }
...
//  Serial.println();
  ...
  value = value/maxBit*20-10;
  return value;
}


bool firstloop=0;// this flag can be set to 1 to init dacs to zero on reset.
void loop() {
